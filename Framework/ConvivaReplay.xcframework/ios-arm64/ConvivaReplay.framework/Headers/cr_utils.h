 ...
 */
static inline void convivaReplay_ASSERT(bool condition, const char *file, int line) {
    if (!condition) {
        fprintf(stderr, "[ConvivaReplay] Warning: Assertion failed at %s:%d\n", file, line);
    }
}

/**
 * Macro to capture FILE and LINE for `convivaReplay_ASSERT`.
 */
#define ASSERT(condition) convivaReplay_ASSERT((condition) ? true : false, __FILE__, __LINE__)
